$(document).ready(function () {
  ...
  var numberRoomsImg = $('.number-rooms__carousel-img').on('initialized.owl.carousel changed.owl.carousel', function (e) {
    if (!e.namespace) return;
    var carousel = e.relatedTarget;
    $('#numberRoomsCounter big').html(carousel.relative(carousel.current()) + 1);
    $('#numberRoomsCounter small').html('-' + carousel.items().length);
  }).owlCarousel({
    items: 1,
    loop: true,
    dots: false,
  ...
  var numberRoomsText = $('.number-rooms__carousel-info').owlCarousel({
    items: 1,
    loop: true,
    dots: false,
    nav: false,
  ...
  owlPrev.click(function () {
    numberRoomsText.trigger('prev.owl.carousel');
  });
  owlNext.click(function () {
    numberRoomsText.trigger('next.owl.carousel');
  });

  var numberRoomsCarousel = $('.number-rooms__carousel');
  numberRoomsCarousel.hover(
      function () {
        numberRoomsImg.trigger('stop.owl.autoplay');
        numberRoomsText.trigger('stop.owl.autoplay');
      },
      function () {
        numberRoomsImg.trigger('play.owl.autoplay', [0, 400]);
        numberRoomsText.trigger('play.owl.autoplay', [0, 400]);
      }
  );
  numberRoomsCarousel.hover(
      function () {
  ...
  // Кнопка для сворачивания блока с инфой о номерах
  var buttonNumberRoomsCarousel = $('.number-rooms__button');

  buttonNumberRoomsCarousel.click(function () {
    $('.number-rooms__carousel-info').toggleClass('collapse');
  })
